  ...
  export default async function CareersPage({ params }) {
    ...
    if (!career) {
      ...
      return(
        ...
        <h2 className='job-title paddingLeft fw-bold' >{career.fields.jobPost}</h2>
        <div className="job-content paddingLeft">
            {career.fields.descr !== "" && (
                <>
                    <h4>Job Description:</h4>
                    <div className='job-description' >{career.fields.descr}</div>
                </>
            )}
            {career.fields.jobResponsibilities !== "" && (
                <>
                    <h4>Job Responsibilities:</h4>
                    <div className='job-responsibilities'>{career.fields.jobResponsibilities}</div>
                </>
            )}
            {career.fields.jobType !== "" && (
                <>
                    <p className='job-type'><strong>Job Type: </strong>{career.fields.jobType}</p>
                </>
            )}
            {career.fields.jobType !== "" && (
                <>
                    <p className='job-type'><strong>Salary: </strong>{career.fields.jobSalary}</p>
                </>
            )}
            <Link href={"/careers/jobform"}className='dBtn btnSecondary mt-4 hasShadow' onClick={scrollTop} >Apply Now</Link>
        </div>
